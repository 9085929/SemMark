import samplingg_utils as sampling_utils
import torch
...
def reject_close_generation(lsh_model, sents, margin, cutoff=None):
    ...
    current_device = lsh_model.device if hasattr(lsh_model, 'device') else 'cuda'
    embeds = torch.tensor(embeds, device=current_device)
    normals = torch.tensor(lsh_model.hasher.normals, device=current_device)
    if cutoff != None:
...
def semstamp_official_completion(
        prompt: str,
    ...
    while True:
        ...
        if lsh_candidate not in accept_mask and current_trials < MAX_TRIALS:
            continue
