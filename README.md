...
2. 微调稳健的句子嵌入器，用具有高余弦相似性的句子嵌入对语义相似的句子进行编码。
3. LSH partitions the embedding space through fixing random hyperplanes and assigning signatures of a vector based on the signs of its dot product with the hyperplanes.
4. LSH 通过固定随机超平面并根据向量与超平面的点积符号来分配向量的签名，从而对嵌入空间进行划分。 
5. Given input sentence $s_1$, repeat generating sentences $s_t$ until $LSH(s_t) \in valid(s_{t-1})$, $t=2,...$ and stop when the max_new_tokens is reached. The valid mask is controlled by the LSH signature of $s_{t-1}$. 
6. 给定输入句子 $s_1$，重复生成句子 $s_t$，直到 $LSH(s_t) \in valid(s_{t-1})$，$t=2,...$，并在达到 max_new_tokens 时停止。有效掩码由 $s_{t-1}$ 的 LSH 签名控制。
...
### Sample usage
1. create data/ directory and load c4_data, which would also create the human subset for evaluation:```python load_c4.py```
